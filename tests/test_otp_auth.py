...
async def test_request_otp_without_redis_always_sends(db, gotrue_stub):
    svc = OtpAuthService(db, None)
    r1 = await svc.request_otp("a@b.com", "1.2.3.4")
    await svc.request_otp("a@b.com", "1.2.3.4")
    assert r1.retry_after_seconds == RESEND_COOLDOWN_S
    assert len(gotrue_stub.sends) == 2  # no cooldown enforced when Redis is down
...
async def test_verify_success_bootstraps_new_user(db, seed, gotrue_stub):
    ...
    resp = await svc.verify_otp("New@Example.com ", "123456")
...
async def test_verify_success_returning_user_not_new(db, seed, gotrue_stub):
    ...
    svc = OtpAuthService(db, FakeRedis())
    gotrue_stub.verify_result = _session(uid)
    resp = await svc.verify_otp("a@b.com", "123456")
...
async def test_verify_wrong_code_reports_attempts_remaining(db, gotrue_stub):
    ...
    with pytest.raises(HTTPException) as exc:
        await svc.verify_otp("a@b.com", "000000")
...
async def test_verify_locks_out_after_max_attempts(db, gotrue_stub):
    ...
    with pytest.raises(HTTPException) as exc:
        await svc.verify_otp("a@b.com", "123456")
...
async def test_verify_without_redis_returns_generic_invalid(db, gotrue_stub):
    svc = OtpAuthService(db, None)
    gotrue_stub.verify_result = None
    with pytest.raises(HTTPException) as exc:
        await svc.verify_otp("a@b.com", "000000")
    assert exc.value.status_code == 401
    assert exc.value.detail == {"code": "invalid_code"}  # no wrong/expired split
