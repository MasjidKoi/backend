...
async def test_noncounting_photos_are_excluded(db, seed):
    ...
    await _add_photo(
        db, masjid.masjid_id, uploaded_by=user,
        source=PhotoSource.COMMUNITY, status=PhotoModerationStatus.PENDING,
    )
    await _add_photo(
        db, masjid.masjid_id, uploaded_by=user,
        source=PhotoSource.COMMUNITY, status=PhotoModerationStatus.REJECTED,
    )
    await _add_photo(
        db, masjid.masjid_id, uploaded_by=user,
        source=PhotoSource.ADMIN, status=PhotoModerationStatus.APPROVED,
    )
    await _add_photo(
        db, masjid.masjid_id, uploaded_by=other,
        source=PhotoSource.COMMUNITY, status=PhotoModerationStatus.APPROVED,
    )
    await seed.commit()
